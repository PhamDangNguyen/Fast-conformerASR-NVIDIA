...
def normalize_vn_tone_for_word(word):
    ...
    if len(nguyen_am_index) == 2:
        if nguyen_am_index[-1] == len(chars) - 1: # 2 nguyen am va ky tu cuoi cung la nguyen am
            x, y = nguyen_am_to_ids[chars[nguyen_am_index[0]]]
            chars[nguyen_am_index[0]] = bang_nguyen_am[x][dau_cau]
        else: # 2 nguyen am + phu am
...
def normalize_vn_tone(sentence):
    """
    ...
    for index, word in enumerate(words):
        ...
        # print(cw)
        if len(cw) == 3:
            cw[1] = normalize_vn_tone_for_word(cw[1])
        words[index] = ''.join(cw)
    
    return unicodedata.normalize("NFKC", ' '.join(words))
